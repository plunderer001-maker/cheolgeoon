...
const serviceCopy = {
  "상가-철거": {
    ...
    situations: ["임대차 종료 전 원상복구 범위를 정해야 하는 경우", "기존 매장 인테리어와 집기를 함께 정리해야 하는 경우"],
    factors: ["붙박이 집기와 간판 철거 범위", "바닥·천장 마감재 종류", "건물 공용부 보양과 반출 동선"],
    prepList: ["임대인 원상복구 요청사항", "매장 전체와 세부 마감 사진", "철거 희망일과 영업 종료일"],
  },
  "상가-철거-비용": {
    ...
    situations: ["대략적인 철거 비용 범위를 먼저 알고 싶은 경우", "방문 견적 전에 비용이 달라지는 항목을 정리하고 싶은 경우"],
    factors: ["평수보다 실제 철거할 마감 범위", "폐기물 양과 차량 접근성", "야간·주말 작업 가능 여부"],
    prepList: ["평수와 업종", "철거할 부분과 남길 부분", "사진 자료와 희망 일정"],
  },
  "사무실-철거": {
    ...
    situations: ["퇴실 전 파티션과 바닥 원상복구가 필요한 경우", "업무 종료 일정에 맞춰 내부 철거를 끝내야 하는 경우"],
    factors: ["파티션 수와 유리 칸막이 여부", "OA 바닥과 전기 배선 정리 범위", "엘리베이터 사용과 작업 시간 제한"],
    prepList: ["사무실 평면 또는 전체 사진", "파티션·회의실·바닥 사진", "퇴실일과 건물 작업 가능 시간"],
  },
  "식당-철거": {
    ...
    situations: ["폐업 후 주방 설비와 홀 인테리어를 같이 정리해야 하는 경우", "덕트·후드·배수 설비 철거 범위가 불명확한 경우"],
    factors: ["후드와 덕트 철거 길이", "주방 타일과 방수층 상태", "냉장고·화구 등 중량 설비 반출"],
    prepList: ["주방 설비와 덕트 사진", "가스·전기 차단 가능 여부", "홀과 주방의 전체 구조"],
  },
  "카페-철거": {
    ...
    situations: ["카운터와 붙박이 가구를 철거해야 하는 경우", "다음 임차인에게 남길 부분과 철거할 부분을 구분해야 하는 경우"],
    factors: ["카운터 고정 방식", "급배수 배관 위치", "조명·바닥·벽면 마감 범위"],
    prepList: ["카운터 안쪽과 외부 사진", "남길 집기와 철거할 집기 목록", "배관·전기 설비 주변 사진"],
  },
  "학원-철거": {
    ...
    situations: ["강의실 칸막이와 상담실 구조를 철거해야 하는 경우", "수업 종료 후 짧은 기간 안에 퇴실 정리가 필요한 경우"],
    factors: ["강의실 수와 칸막이 재질", "방음재와 게시판 철거 여부", "복도 보양과 폐기물 반출 시간"],
    prepList: ["강의실별 사진", "수업 종료일과 퇴실일", "남길 집기와 철거할 집기 구분"],
  },
  "원상복구-철거": {
    ...
    situations: ["임대인이 요구한 원상복구 범위를 확인해야 하는 경우", "철거와 복구 기준을 나눠 견적을 보고 싶은 경우"],
    factors: ["계약서 원상복구 조항", "벽·바닥·천장 훼손 정도", "임대인 확인이 필요한 잔여 시설"],
    prepList: ["계약서 원상복구 조항", "현재 훼손 부위 사진", "임대인 또는 관리실 요청사항"],
  },
  "폐업-철거": {
    ...
    situations: ["폐업 일정에 맞춰 집기 정리와 철거를 같이 진행해야 하는 경우", "직접 처리할 물품과 철거 공정에 포함할 물품을 나눠야 하는 경우"],
    factors: ["남은 집기와 폐기물 양", "폐업 후 작업 가능 기간", "반출 차량 진입과 엘리베이터 사용"],
    prepList: ["폐업 예정일", "남은 집기와 폐기물 사진", "직접 가져갈 물품 목록"],
  },
  "철거-비용": {
  ...
  "철거-업체": {
    ...
    situations: ["어떤 업체에 맡겨야 할지 기준이 필요한 경우", "단순 폐기물 처리인지 철거 공정인지 구분이 필요한 경우"],
    factors: ["작업 가능 범위와 제외 조건", "방문 견적 가능 여부", "일정 대응과 현장 관리 방식"],
    prepList: ["현장 주소와 내부 사진", "원하는 작업 범위", "상담 가능한 시간과 희망 일정"],
  },
} as const;
  ...
  return (
    <main className="seo-main">
      ...
      <section className="seo-section seo-scenario">
        ...
        <h2>{region.name}에서 {page.service.keyword} 상담을 많이 요청하는 경우입니다.</h2>
        <p>
          철거 문의는 단순히 철거할 면적만으로 판단하기 어렵습니다. 임대차
          원상복구 기준, 남겨야 할 시설, 반출 동선, 작업 가능 시간까지 같이
          봐야 실제 견적과 일정이 현실적으로 정리됩니다.
        </p>
        <div>
          {copy.situations.map((situation) => (
            <article key={situation}>
              <h3>{situation}</h3>
              <p>
                이런 경우에는 현장 사진만으로도 1차 상담을 시작할 수 있습니다.
                다만 관리 규정이나 설비 철거 범위가 애매하면 방문 확인이
                필요할 수 있습니다.
              </p>
            </article>
          ))}
      ...
      <section className="seo-section seo-two-column">
        <div>
          ...
          <h2>{page.keyword} 비용을 볼 때 먼저 나눠야 할 항목입니다.</h2>
          <p>
            같은 {page.service.keyword} 문의라도 실제 견적은 현장 조건에 따라
            달라집니다. 아래 항목을 먼저 확인하면 상담 과정에서 빠지는 부분을
            줄일 수 있습니다.
          </p>
        </div>
        <div className="seo-check-list">
          {copy.factors.map((factor) => (
            <article key={factor}>
              ...
              <h3>{factor}</h3>
              <p>
                사진으로 확인 가능한 부분은 먼저 보고, 현장 판단이 필요한
                부분은 방문 견적 여부를 안내합니다.
              </p>
            </article>
          ))}
      ...
      <section className="seo-section seo-detail-grid">
        {copy.prepList.map((item) => (
          <article key={item}>
            ...
            <h2>{item}</h2>
            <p>
              정확하지 않아도 괜찮습니다. 현재 알고 있는 범위만 남겨주시면
              상담 과정에서 필요한 정보를 다시 정리해드립니다.
            </p>
          </article>
        ))}
        <article>
          ...
          <p>
            부분철거만 따로 진행하거나 폐기물 처리만 단독으로 요청하는 경우는
            진행이 어렵습니다. 전체 철거 공정 안에 포함되는 경우 현장 조건을
            보고 상담합니다.
          </p>
        </article>
      </section>
      ...
      <section className="seo-section seo-faq">
        ...
        <div className="seo-faq-list">
          <article>
            ...
            <p>
              사진으로 1차 범위는 볼 수 있지만 정확한 견적은 현장 조건에 따라
              방문 확인이 필요할 수 있습니다. 특히 반출 동선이나 설비 철거가
              포함되면 현장 확인이 더 중요합니다.
            </p>
          </article>
          <article>
            ...
            <p>
              전국 상담을 기준으로 안내합니다. 다만 도서산간 지역은 방문 또는
              출동 비용이 발생할 수 있어 주소 확인 후 가능 여부를 안내합니다.
            </p>
          </article>
          <article>
            <HelpCircle size={22} aria-hidden="true" />
            ...
            <p>
              조건에 따라 가능 여부와 준비 서류가 달라집니다. 확정이나 보장
              표현이 아니라, 상담 과정에서 신청 가능성을 확인하는 방식으로
              안내합니다.
            </p>
          </article>
        </div>
